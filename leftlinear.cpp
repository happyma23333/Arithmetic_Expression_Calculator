...
struct token {
	// 0-关键字，1-变量， 2-数字，3-临时变量, 4-非终结符, 5-运算, 6-#
	int code;
	int value;
};
...
int ll1table[5][8] = {1, 0, 0, 0, 0, 1, 0, 0,
	0, 2, 3, 0, 0, 0, 4, 4,
	5, 0, 0, 0, 0, 5, 0, 0,
	0, 8, 8, 6, 7, 0, 8, 8,
	9, 0, 0, 0, 0, 10, 0, 0};
...
struct token E = {4, 0}, E1 = {4, 1}, T = {4, 2}, T1 = {4, 3}, F = {4, 4}; // 各推导式
...
void act(int s) //根据状态转移矩阵来生成字符序列对应token
{
	int value;
	switch (s)
	{
...
void pushSYN(int i) {
	struct token p = {0, 1}, mi = {0, 2}, mul = {0, 3}, di = {0, 4}, lb = {0, 5}, rb = {0, 6}; //+-*/()
	struct token var = {1, };
	switch (i) {
		case 0: printf("语法错误\n");
			exit(1);
...
int mapTokenToCol(struct token t) {
	if (t.code == 1 || t.code == 2) 
		return 0;
	else if (t.code == 0) 
		return t.value;
	else if (t.code == 6)
...
void ll() {
	...
	while(!SYN.empty()) {
		...
		// printf("%ld\n", SYN.size());
		if (t.code == 0) { // 运算符
			if (tok[i_token].code == 0 && t.value == tok[i_token].value) { //匹配
				i_token++;
			} else {
				printf("语法错误\n");
				exit(1);
			}
		} else if (t.code == 1) { //变量或数字
			// printf("(%d, %d)\n", tok[i_token].code, tok[i_token].value);
			if (tok[i_token].code == 1 || tok[i_token].code == 2) { // 匹配
			...
			QUAT(t.value);
		} else if (t.code == 4) { //非终结符
			pushSYN(ll1table[t.value][mapTokenToCol(tok[i_token])]);
		} else if (t.code == 6) { //#
			if (tok[i_token].code == 6)
				break;
			else {
				printf("语法错误\n");
				exit(1);
